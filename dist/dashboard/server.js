...
export const startDashboardServer = (client) => {
    ...
    app.get('/api/guilds/:id/autoroles', requireAuth, async (req, res) => {
        try {
            const autoroles = await prisma.autorole.findMany({ where: { guildId: req.params.id } });
            res.json(autoroles.map((ar) => ar.roleId));
        }
        catch (error) {
            res.status(500).json({ error: 'Failed to fetch autoroles' });
        }
    });
    app.post('/api/guilds/:id/autoroles', requireAuth, async (req, res) => {
        const guildId = req.params.id;
        const { roleIds } = req.body;
        try {
            await prisma.$transaction([
                prisma.autorole.deleteMany({ where: { guildId } }),
                ...roleIds.map((roleId) => prisma.autorole.create({ data: { guildId, roleId } })),
            ]);
            res.json({ success: true });
        }
        catch (error) {
