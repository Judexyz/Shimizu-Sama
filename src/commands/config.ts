...
const command: Command = {
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    if (group === 'welcome' || group === 'goodbye') {
      ...
      if (!config) config = await prisma.welcomeConfig.create({ data: { guildId } });
    ...
    if (group === 'autorole') {
      ...
      if (subCommand === 'list') {
        ...
        if (roles.length === 0) {
          ...
          await interaction.followUp(`**Autoroles:**\n${roles.map((r: any) => `<@&${r.roleId}>`).join('\n')}`);
        }
      }
    }

    if (group === 'raid') {
      let config = await prisma.raidProtection.findUnique({ where: { guildId } });
      if (!config) config = await prisma.raidProtection.create({ data: { guildId } });

      if (subCommand === 'enable' || subCommand === 'disable') {
        const enabled = subCommand === 'enable';
        await prisma.raidProtection.update({ where: { guildId }, data: { enabled } });
        await interaction.followUp(`✅ Successfully ${enabled ? 'enabled' : 'disabled'} raid protection.`);
      }
