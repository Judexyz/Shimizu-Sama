import { ChatInputCommandInteraction, SlashCommandBuilder, PermissionFlagsBits, TextChannel, EmbedBuilder, GuildMember } from 'discord.js';
...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub => 
      sub.setName('create')
        ...
        .addStringOption(opt => opt.setName('duration').setDescription('e.g., 30s, 10m, 2h, 7d, 1w').setRequired(true))
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    if (!channel || !(channel instanceof TextChannel)) {
      await interaction.reply({ content: '❌ Giveaways must be managed in a text channel.', ephemeral: true });
      return;
    }
    ...
    if (subCommand === 'create') {
      ...
      if (!durationMs) {
        await interaction.reply({ content: '❌ Invalid duration format. Use 30s, 10m, 2h, 7d, or 1w.', ephemeral: true });
        return;
      }
    ...
    if (subCommand === 'end') {
      ...
      if (!giveaway || giveaway.status !== 'ACTIVE') {
        await interaction.reply({ content: '❌ Active giveaway not found with that message ID.', ephemeral: true });
        return;
      }
    ...
    if (subCommand === 'cancel') {
      ...
      if (success) {
        await interaction.followUp({ content: '✅ Giveaway cancelled successfully.', ephemeral: true });
      }
    }
    ...
    if (subCommand === 'reroll') {
      ...
      if (!giveaway || giveaway.status !== 'COMPLETED') {
        await interaction.reply({ content: '❌ Completed giveaway not found with that message ID.', ephemeral: true });
        return;
      }

      await interaction.deferReply({ ephemeral: false }); // Public reply for reroll
      const newWinners = await GiveawayService.rerollGiveaway(giveaway.id, interaction.client);
      
      if (newWinners.length > 0) {
        await interaction.followUp(`🎉 New giveaway winner(s): ${newWinners.map(w => `<@${w}>`).join(', ')}! Congratulations!`);
      } else {
    ...
    if (subCommand === 'list') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(0x0099ff)
        .setDescription(
          activeGiveaways.map(g => `**${g.prize}** in <#${g.channelId}>\nEnds: <t:${Math.floor(g.endsAt.getTime() / 1000)}:R> | Message ID: \`${g.messageId}\``).join('\n\n')
        );
