...
export const startDashboardServer = (client: ShimizuClient) => {
  ...
  app.get('/api/auth/callback', async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign(
        { id: userData.id, username: userData.username, avatar: userData.avatar, access_token: tokenData.access_token },
        JWT_SECRET,
        { expiresIn: '7d' }
      );
  ...
  app.get('/api/guilds', requireAuth, async (req: any, res) => {
    try {
      ...
      if (!isBotOwner) {
        manageableGuilds = guilds.filter((g: any) => (parseInt(g.permissions) & MANAGE_GUILD) === MANAGE_GUILD);
      }
  ...
  app.get('/api/guilds/:id/settings', requireAuth, async (req: any, res) => {
    ...
    if (!client.guilds.cache.has(guildId)) return res.status(404).json({ error: 'Guild not found' });
  ...
  app.post('/api/guilds/:id/settings', requireAuth, async (req: any, res) => {
    ...
    const { 
      levelingEnabled, economyEnabled, levelUpChannelId, levelUpMessage, prefix,
      categoryId, supportRoleId, transcriptChannelId 
    } = req.body;
